...
{
    /// <summary>
    /// Script para probar el sistema de voz real con Windows
    /// </summary>
    public static class TestWindowsVoiceSystem
    {
        [MenuItem("Tools/VoiceSystem/Test Windows Voice System")]
        public static void RunTestWindowsVoiceSystem()
        {
            Debug.Log("=== PROBANDO SISTEMA DE VOZ REAL WINDOWS ===");
        ...
        [MenuItem("Tools/VoiceSystem/Stop Windows Voice System")]
        public static void StopWindowsVoiceSystem()
        {
            var manager = Object.FindFirstObjectByType<VoiceSystemManager>();
            if (manager != null)
            {
        ...
        [MenuItem("Tools/VoiceSystem/List Available Voices")]
        public static void ListAvailableVoices()
        {
            ...
            if (manager != null)
            {
                ...
                if (tts != null)
                {
                    ...
                    string voiceList = "Voces disponibles:\n\n";
                    for (int i = 0; i < voices.Length; i++)
                    {
                        voiceList += $"{i + 1}. {voices[i]}\n";
                    }
                    
                    EditorUtility.DisplayDialog("Voces Disponibles", voiceList, "OK");
                    Debug.Log($"Voces disponibles: {string.Join(", ", voices)}");
                }
            }
        }
    }
